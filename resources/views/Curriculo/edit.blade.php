...
@csrf
    ...
    <div class="container mt-5">
        ...
        <div class="form-floating mb-3">
            <input type="text" class="form-control" name="Nome" id="Nome" placeholder="Nome" value="{{$curriculo->Nome}}" require>
        ...
        <div id="BoxPeriodos">
        <?php $periodoId = 0; foreach($periodos as $nome => $periodo): ?>

            <div class="row" id="Periodo_{{$periodoId}}">
                <div class="col-2 BoxNomePeriodo">
                    <div class="form-floating mb-3 mt-3">
                        <input type="text" class="form-control" name="NomePeriodo[{{$periodoId}}]" value="{{$nome}}" placeholder="Nome Periodo" required>
                        <label for="NomePeriodo[{{$periodoId}}]">Nome Periodo</label>
                    </div>
                </div>
                <div class="col-9">
                    <div id="BoxDisciplinas_{{$periodoId}}" class="row">
                    <?php $disciplinaId = 0; foreach($periodo as $disciplinaPeriodo): ?> 
                        <div class="col-2" id="Disciplina_div_{{$disciplinaId}}">
                            <select id="Disciplina_Select_{{$disciplinaId}}" class="form-control selectpicker mb-3" style="width: 100px;" aria-label="Default select" name="Disciplina[{{$periodoId}}][{{$disciplinaId}}]" data-live-search="true">
                                <?php foreach($Disciplinas as $disciplina): ?>
                                    <option style="width:inherit" data-tokens="<?= $disciplina->Codigo ?>" <?php if($disciplinaPeriodo->id == $disciplina->id) echo 'selected'; ?> value="<?=$disciplina->id?>"><?= $disciplina->Codigo ?></option>
                                <?php endforeach; ?>
                            </select>
                            <i class="fa fa-minus-circle remove flex" onclick="remover_disciplina({{$disciplinaId}})" aria-hidden="true"></i>
                        </div>
                    <?php $disciplinaId++; endforeach; ?>
                    </div>
                </div>
                <div class="col-1 BoxAdd">
                    <div class="flex pt-2"> <i class="fa fa-plus-circle add" onclick="adicionar_disciplina({{$periodoId}})" aria-hidden="true"></i> </div>
                    <div class="flex pt-1"> <i class="fa fa-minus-circle remove" onclick="remover_periodo({{$periodoId}})" aria-hidden="true"></i> </div>
                </div>
                <hr/>
            </div>

        <?php $periodoId++; endforeach; ?>
        </div>
...
@section('script')
    ...
    <script>
        var idPeriodo = {{$periodoId}};
        var idDisciplina = {{$disciplinaId}};
