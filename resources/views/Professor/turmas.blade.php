...
@section('content')
    ...
    <div class="container mt-5">
        <button type="button" class="btn btn-outline-success float-end" onclick="redirecionar('<?= url('/turma/add/'.$professor->id) ?>')">Registrar Turma</button>
        <h1>Turmas do Professor - {{ $professor->Nome }}</h1>
        ...
        <table class="table">
            ...
            <?php foreach($turmas as $turma):?>
                <tr>
                    ...
                    <th scope="row">{{$turma->MaximoAlunos}}</th>
                    <th scope="row">{{$turma->Ano}}.{{$turma->Semestre}}</th>
                    <th scope="row">{{$turma->disciplina->Codigo}}</th>
                    <td class="float-end">
                        <button type="button" class="btn btn-outline-warning" onclick="redirecionar('<?= url('/turma/edit/'.$turma->id.'/'.$professor->id) ?>')">Editar</button>
